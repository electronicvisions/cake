...
"""
Neurons with E_l over V_t and self spike stimulation
...
class FindRepeaterNeurons(object):
    ...
    def run(self, wafer_coordinate, hicann_coordinate, neurons):
        ...
        for neuron in neurons:
            ...
            wafer.restart(runner)
            trace = adc.getTimestamps(), adc.trace()
            adc.freeHandle()

            tmp = hicann.receivedSpikes(link_out)
            spikes = tmp[tmp[:, 1] == int(self.neuron_l1address)][:, 0]

            data = (trace, spikes)
            numpy.save('hicann_l1_delay_N{:0>3}'.format(int(neuron.id())), data)

            if link_out == GbitLinkOnHICANN(7):
                hicann.analog.set_preout(self.analog)
                wafer.configure(self.UpdateAoutCFG())
                adc = hicann.analogRecorder(self.analog)
                adc.activateTrigger(self.runtime)
                wafer.restart(runner)
                trace = adc.getTimestamps(), adc.trace()
                ...
                spikes = tmp[tmp[:, 1] == int(self.neuron_l1address)][:, 0]
                data = (trace, spikes)
                numpy.save('hicann_l1_preout_delay_N{:0>3}'.format(int(neuron.id())), data)
...
def main():
    ...
    parser.add_argument('--adc-freq', action='store', type=float, required=True,
    ...
    #cmd.run(args.wafer, args.hicann, N[3:4])
    cmd.run(args.wafer, args.hicann, N[224:256])
