...
wafer_with_backend = pyredman.load.WaferWithBackend(args.defects_path, C.Wafer(args.wafer), ignore_missing=True)
...
for hicann in iter_all(C.HICANNOnWafer):
    ...
    if (not wafer_with_backend.hicanns().has(hicann)):
        continue
    jtag[hicann_id] = not fpga_with_backend.hslinks().has(hicann.toHighspeedLinkOnDNC())
    ...
    for res in enum_resources + non_enum_resources:
        exec ("has_value = hicann_with_backend.{}().has_value()".format(res[0]))
        if not has_value:
            continue
        exec ("{}[hicann_id] = hicann_with_backend.{}().available()".format(res[0], res[0]))

# generate JTAG with green yellow
cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["green","yellow"])
max_value = 1
jtag_figure = pycake.helpers.plotting.get_bokeh_figure("JTAG", jtag, max_value, cmap, default_fill_color = 'blue')
# generate all other colormaps
cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["green", "red"])

max_value = 1
blacklisted_figure = pycake.helpers.plotting.get_bokeh_figure("blacklisted", blacklisted, max_value, cmap, default_fill_color = 'blue')

cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", "green"])

for res in enum_resources:
    exec("max_value = C.{}OnHICANN.enum_type.end".format(res[1]))
    exec("""{}_figure = pycake.helpers.plotting.get_bokeh_figure("{}",{}, max_value, cmap, add_text = {}, default_fill_color = 'blue')""".format(res[0], res[0], res[0], res[2]))
...
pycake.helpers.plotting.store_bokeh("Redman Wafer {} Overview".format(args.wafer), figures, "redman_w{}.html".format(args.wafer))
